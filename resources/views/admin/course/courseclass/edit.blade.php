...
<section class="content">
  ...
  <div class="row">
    <div class="col-md-12" style = "padding-left :20px">
      ...
      <div class="box box-primary">
        <div class="box-header with-border">
          <h3 class="box-title">{{ __('adminstaticword.VideoQuizQuestion') }}</h3>
        </div>
        <div class="box-body">
          <a data-toggle="modal" data-target="#myModalSubtitle1" href="#" class="btn btn-info btn-sm">+ {{ __('adminstaticword.Add') }} {{ __('adminstaticword.VideoQuizQuestion') }}</a>

          <!--Model start-->
          <div class="modal fade" id="myModalSubtitle1" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
            <div class="modal-dialog modal-lg" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-="true">&times;</span></button>
                  <h4 class="modal-title" id="myModalLabel"> {{ __('adminstaticword.Add') }} {{ __('adminstaticword.VideoQuizQuestion') }}</h4>
                </div>
                <div class="box box-primary">
                  <div class="panel panel-sum">
                    <div class="modal-body">
                      <form enctype="multipart/form-data" id="course_question_form" method="post" action="/admin/class/<?php echo $cate->id ?>/addcoursevideoquiz" data-parsley-validate class="form-horizontal form-label-left">
                        {{ csrf_field() }}

                        <div id="subtitle">

                          <label>{{ __('adminstaticword.VideoQuizQuestion') }}:</label>
                          <br><br>
                          <div class="row">
                            <div class="col-md-6">
                              <label for="exampleInputDetails">{{ __('adminstaticword.Question') }}:<sup class="redstar">*</sup></label>
                              <textarea rows="1" name="question" id="question" class="form-control" ></textarea>
                            </div>
                            <div class="col-md-6">
                              <div>
                                ...
                                <br>
                                <label for="">A:</label>
                                <input type="text" name="A" id="answerA" class="form-control" autocomplete="off">
                                <label for="">B:</label>
                                <input type="text" name="B" id="answerB" class="form-control" autocomplete="off">
                                <label for="">C:</label>
                                <input type="text" name="C" id="answerC" class="form-control" autocomplete="off">
                                <label for="">D:</label>
                                <input type="text" name="D" id="answerD" class="form-control" autocomplete="off">
                          ...
                          <div class="row">
                            <div class="col-md-6">
                              <label>{{ __('adminstaticword.TimeFrame') }}:<sup class="redstar">*</label>
                              <p>Please write in format <b><i>mm:ss</i></b>. for eg: <b><i>00:12</i></b></p>
                              <input type="text" name="timeframe" id="timeframe" class="form-control" autocomplete="off" required>
                            </div>
                            <div class="col-md-6">
                              <div>
                                <label for="correctanswer">{{ __('adminstaticword.CorrectAnswer') }}:<sup class="redstar">*</label>
                                <br>
                                <select style="width:60px" name="correctanswer" id="correctanswer">
                                  <option value="A" id="A">A</option>
                                  <option value="B" id="B">B</option>
                                  <option value="C" id="C">C</option>
                                  <option value="D" id="D">D</option>
                                </select>
                                <!-- <input type="text" name="correctanswer" class="form-control" autocomplete="off" required> -->
                              </div>
          ...
          <table id="tableQuiz" class="table table-bordered table-striped">
            <thead>
              <br>
              <br>
              <tr>
                ...
                <th>{{ __('adminstaticword.TimeFrame') }} </th>
            ...
            <tbody>
              <?php $i = 0; ?>
              @foreach($course_video_quiz as $quiz)
              <?php $i++; ?>
              <tr class="quiz">
                <td><?php echo $i; ?></td>
                <td>{{$quiz->question}}</td>
                ...
                <td>{{$quiz->d}}</td>
                <td>{{$quiz->answer}}</td>
                <td>{{$quiz->timeframe}}</td>
                <td>
                  <?php $quizDetail = $quiz->id.';;'.$quiz->question.';;'.$quiz->a.';;'.$quiz->b.';;'.$quiz->c.';;'.$quiz->d.';;'.$quiz->answer.';;'.$quiz->timeframe;?>
                  <button data-target-id="{{$quizDetail}}"  data-toggle="modal" data-target="#myModalSubtitle1" href="#" class="btn btn-success btn-sm display-inline"><i class="glyphicon glyphicon-pencil"></i></button>                   
                </td>
                <td>
                  <form method="post" action="/admin/class/<?php echo $quiz->id ?>/deletecoursevideoquiz" data-parsley-validate class="form-horizontal form-label-left">
                    {{ csrf_field() }}
      ...
      <table id="example1" class="table table-bordered table-striped">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              <form method="post" action="{{ route('del.subtitle',$subtitle->id) }}" data-parsley-validate class="form-horizontal form-label-left">
                {{ csrf_field() }}
...
<script>
   $(document).ready(function () {
        $("#myModalSubtitle1").on("show.bs.modal", function (e) {
            var quizDetail = $(e.relatedTarget).data('target-id');
            if(quizDetail != undefined)
            {
              var quizDetail = quizDetail.split(";;");
              $('#answerA').val(quizDetail[2]);
              $('#answerB').val(quizDetail[3]);
              $('#answerC').val(quizDetail[4]);
              $('#answerD').val(quizDetail[5]);
              $('#timeframe').val(quizDetail[7]);
              $(tinymce.get('question').getBody()).html(quizDetail[1]);
              document.getElementById("correctanswer").value = quizDetail[6];
              $("#course_question_form").attr("action", "/admin/class/"+ quizDetail[0] +"/editcoursevideoquiz"); 
            }
