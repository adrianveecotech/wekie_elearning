...
<head>
	...
	<script>
		...
		function check_answer(correctAnswer) {
			try {
				var d = document.querySelector('input[name^="answer"]:checked').value;
			} catch (err) {
			...
			if (d != undefined) {
				if (d != currentQuestion.answer) {
					var j = document.getElementById('correctAnswer');
					console.log(j);
					document.getElementById('correctAnswer').style.display = "none";
					document.getElementById('emptyAnswer').style.display = "none";
					document.getElementById('wrongAnswer').style.display = "block";
					document.getElementById('buttonSubmit').style.display = "none";
					document.getElementById('wrongAnswer').innerHTML = "Correct answer should be " + correctAnswer + "!";
					question.push('false');

				} else if (d == currentQuestion.answer) {
					var j = document.getElementById('correctAnswer');
					console.log(j);
		...
		(function($) {
			...
			$(document).ready(function() {
				var video_quiz = <?php echo json_encode($video_quiz); ?>;
					...
					for (var i = 0; i < tt; i++) {
						...
						$.ajax({
							...
							success: function(data) {
								if(oldTime != video['curTime']){
									video_quiz.forEach(element => {
										console.log('timeframe set : ' + element.timeframe);
										console.log('video time : ' + video['curTime']);

										if (element.timeframe == video['curTime']) {
											currentQuestion = element;
											oldTime = video['curTime'];
											$('.collapse in').collapse("show");
											video.pause();
											var player = document.getElementsByClassName('fwduvp');
											// var divtest = document.createElement("div");
											// divtest.innerHTML="new div";

											var htmlstr =
												`<div class="collapse in" id="myModal" role="dialog">
											...
											<div class="modal-content">
												...
												<h4 class="modal-title" style="color:black;">Video Quiz</h4>
												</div>
												<div class="modal-body">
												<h4 style="color:black;font-size: 20px;">` + element['question'] + `</h4>
												<br>
												<div style = "padding-left:50px">
												<label class="radio" style="color:black;font-size: 15px;">
													<input type="radio" required name="answer[` + element['id'] + `]" value="A">` + element['a'] + `
												</label>
												<br>
												<label class="radio" style="color:black;font-size: 15px;">
													<input type="radio" required name="answer[` + element['id'] + `]" value="B">` + element['b'] + `
												</label>
												<br>
												<label class="radio" style="color:black;font-size: 15px;">
													<input type="radio" required name="answer[` + element['id'] + `]" value="C">` + element['c'] + `
												</label>
												<br>
												<label class="radio" style="color:black;font-size: 15px;">
													<input type="radio" required name="answer[` + element['id'] + `]" value="D">` + element['d'] + `
												</label>
												</div>
												<br>
												<br>
												<div id="emptyAnswer" style="display:none;color:red;font-size:15px"><b>Answer is empty!</b></div>
						 						<div id="wrongAnswer" style="display:none;color:red;font-size:15px"><b></b></div>
						 						<div id="correctAnswer" style="display:none;color:green;font-size:15px"><b>Correct Answer!</b></div>
												<br>
												</div>
												<div class="modal-footer">
												<button type="button" id="buttonSubmit" class="btn btn-default" onclick="check_answer('` + element['answer'] + `')">Submit</button>
												<button type="button" class="btn btn-default" onclick="hide()">Close</button>
												</div>
											</div>
											
											</div>
										</div>`;
											player[0].insertAdjacentHTML('beforeend', htmlstr);
										}
									});
								}
							},
...
<style>
	...
	.collapse {
		z-index: 99999999999999999147483647
	}
