...
class WatchController extends Controller
{
    ...
    {
        Session::flush();
        $video_quiz = CourseVideoQuiz::where('courseclass_id', $id)->get();
        foreach ($video_quiz as $value) {

            $split = explode(':', $value->timeframe);
            // if (count($split) == 2) {
                if ($split[1] == '00') {
                    $split[0] = $split[0] - 1;
                    $split[0] = sprintf('%02d', $split[0]);
                    $split[1] = '60';
                };
            // }
            ...
            //     if ($split[2] == '00') {
            //         $split[1] = $split[1] - 1;
            //         $split[1] = sprintf('%02d', $split[1]);
            //         $split[2] = '60';
            //     };
            // }
            $split = implode(":", $split);
            $value->timeframe = $split;
        }
        ...
        if (Auth::check()) {
            ...
            if (Auth::User()->role == "admin") {
                return view('classwatch', compact('class', 'video_quiz'));
            } else {
                if (!empty($order)) {
